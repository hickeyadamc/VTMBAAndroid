...
public class AlumniSearchFragment
...
//        this.requestWindowFeature(Window.FEATURE_NO_TITLE);
        mRootView = inflater.inflate(R.layout.activity_search_menu,container,false);
        state = new String();
        state = "ANY";
        final Button searchAlumni = (Button) mRootView.findViewById(R.id.searchAlumni);
        final Button backButton = (Button) mRootView.findViewById(R.id.backPage);
        ...
        searchMetroArea = (EditText) mRootView.findViewById(R.id.searchMetroArea);

        searchAlumni.setOnClickListener(new View.OnClickListener() {
...
//        });
    ...
    public void goToResults()
    {
        ...
        if(firstName.length()<1 && lastName.length()<1 && location.length()<1 && employer.length()<1 && metroArea.length()<1)
        {
            if (!state.equals("ANY"))
            {
...
//            finish();
    ...
    private void startResultsFragment(ArrayList<String> searchParams) {
        ...
        arguments.putStringArrayList(AlumniResultsListFragment.EXTRA_SEARCH_PARAMETERS, searchParams);
        mMainActivity.switchContent(MainActivity.FRAGMENT_SEARCH_ALUMNI_RESULTS, arguments);
